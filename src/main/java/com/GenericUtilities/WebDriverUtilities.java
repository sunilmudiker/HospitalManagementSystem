...
public class WebDriverUtilities {
	...
	/**
	 * This method is used to print all broken links present in the application.
	 */
	public void getAllBrokenLinks() {
		List<WebElement> allLinks = driver.findElements(By.tagName("a"));
		List<String> workingLinks = new ArrayList<String>();
		List<String> brokenLinks = new ArrayList<String>();
		//to list out all broken links and working links
		for (WebElement links : allLinks) {
			String eachLink = links.getAttribute("href");
			if (eachLink != null) {
				if (eachLink.contains("http")) {
					workingLinks.add(eachLink);
				} else {
					brokenLinks.add(eachLink + "  ==> doesnt contains http protocal");
				}
			} else
				brokenLinks.add(eachLink + "  ==> is Null");
		}
		//to check whether the working links are working or not. if not add it to broken links
		for (String eachLink : workingLinks) {
			try {
				...
				if (code >= 400)
					brokenLinks.add(eachLink + "  ==> responseCode : " + code + "  ==> responseMsg : " + msg);
			} catch (Exception e) {
				brokenLinks.add(eachLink + " ==> not connected");
			}
		}
